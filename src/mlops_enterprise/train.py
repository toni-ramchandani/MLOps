...
def train_and_log(config_path: str = "configs/config.yaml") -> dict:
    ...
    with mlflow.start_run(run_name="logreg_production_style") as run:
        ...
        if metrics["val_roc_auc"] < gate:
            raise RuntimeError(f"QUALITY GATE FAILED: val_roc_auc={metrics['val_roc_auc']:.4f} < {gate:.4f}")

        mlflow.log_params(params)
        mlflow.log_param("random_state", s.random_state)
        mlflow.log_metrics(metrics)

        mlflow.log_dict(json.loads(Path(cfg["data"]["data_meta_path"]).read_text(encoding="utf-8")), "data_meta.json")
        ...
        mlflow.sklearn.log_model(pipe, artifact_path="model", signature=signature, input_example=X_train.iloc[:5])
